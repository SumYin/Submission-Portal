...
import { FILE_CATEGORIES, FileCategoryId, VIDEO_CODECS, AUDIO_CODECS } from "@/lib/fileTaxonomy"
...
export default function FormEditor({ initialValues, onSubmit, isSubmitting, mode, onCancel }: FormEditorProps) {
  ...
  // Reset allowedMimes and customExtensions when kind changes
  useEffect(() => {

    const catId: FileCategoryId =
      kind === "image" ? "images" : kind === "video" ? "video" : kind === "audio" ? "audio" : ("text" as FileCategoryId)
    const validMimes: string[] = [...(FILE_CATEGORIES.find((c) => c.id === catId)?.mimes ?? [])]

    const currentMimes = form.getValues("allowedMimes")
    const newMimes = currentMimes.filter(m => validMimes.includes(m))

    if (newMimes.length !== currentMimes.length) {
      form.setValue("allowedMimes", newMimes)
    }
              ...
              <TabsContent value="constraints" className="space-y-6">
                ...
                {kind === "audio" ? (
                  <div className="space-y-4">
                    ...
                    {/* Channels constraint removed as per user request */}
